...
(function() {
  var addDocumentNodes, addDocumentNodesScheduled, addMap, addNodes, applyChanges, changedTargets, documentObserver, documentObservers, handleAsyncChangeEvents, handleChangeEvent, handleDocumentMutations, initMap, initializerMap, innerHTMLReplacementIsBuggy, removeNodes, revalidateDescendantObservers, revalidateObservers, revalidateOrphanedElements, runAdd, runInit, runRemove, scheduleAddDocumentNodes, selectorSet, stopObserving, uid;

  innerHTMLReplacementIsBuggy = (function() {
    var a, b, c;
  ...
  selectorSet.querySelectorAll = $.find;

  selectorSet.matchesSelector = $.find.matchesSelector;

  initMap = new WeakMap;

  addMap = new WeakMap;

  initializerMap = new WeakMap;

  runInit = function(el, observer) {
    var initIds, initializer;
    if (!(initIds = initMap.get(el))) {
      initIds = [];
      initMap.set(el, initIds);
    }
    if (initIds.indexOf(observer.id) === -1) {
      if (observer.initialize != null) {
        initializer = observer.initialize.call(el, el);
      }
      initializerMap.set(el, initializer);
      initIds.push(observer.id);
    }
  };

  runAdd = function(el, observer) {
    var addIds, initializer, ref, ref1;
    if (!(addIds = addMap.get(el))) {
      addIds = [];
      addMap.set(el, addIds);
    }
    if (addIds.indexOf(observer.id) === -1) {
      observer.elements.push(el);
      if (initializer = initializerMap.get(el)) {
        if (!('length' in initializer)) {
          if ((ref = initializer.add) != null) {
            ref.call(el, el);
          }
        }
      }
      if ((ref1 = observer.add) != null) {
        ref1.call(el, el);
      }
  ...
  runRemove = function(el, observer) {
    var addIds, i, id, index, initializer, len, ref, ref1, ref2, ref3, ref4;
    if (!(addIds = addMap.get(el))) {
      return;
    }
    if (observer) {
      ...
      if (index !== -1) {
        if (initializer = initializerMap.get(el)) {
          if (!('length' in initializer)) {
            if ((ref = initializer.remove) != null) {
              ref.call(el, el);
            }
          }
        }
        if ((ref1 = observer.remove) != null) {
          ref1.call(el, el);
        }
      ...
      for (i = 0, len = ref2.length; i < len; i++) {
        ...
        if (initializer = initializerMap.get(el)) {
          if ((ref3 = initializer.remove) != null) {
            ref3.call(el, el);
          }
        }
        if ((ref4 = observer.remove) != null) {
          ref4.call(el, el);
        }
      }
      addMap["delete"](el);
  ...
  addNodes = function(changes, nodes) {
    var data, descendant, el, elements, i, j, k, l, len, len1, len2, len3, ref, ref1, ref2;
    for (i = 0, len = nodes.length; i < len; i++) {
      el = nodes[i];
      if (!(el.nodeType === Node.ELEMENT_NODE)) {
        continue;
      }
      ref = selectorSet.matches(el);
      for (j = 0, len1 = ref.length; j < len1; j++) {
        data = ref[j].data;
        changes.push(['add', el, data]);
      }
      ref1 = selectorSet.queryAll(el);
      for (k = 0, len2 = ref1.length; k < len2; k++) {
        ref2 = ref1[k], data = ref2.data, elements = ref2.elements;
        for (l = 0, len3 = elements.length; l < len3; l++) {
          descendant = elements[l];
  ...
  removeNodes = function(changes, nodes) {
    var descendant, el, i, j, len, len1, ref;
    for (i = 0, len = nodes.length; i < len; i++) {
      el = nodes[i];
      if (!(el.nodeType === Node.ELEMENT_NODE)) {
        continue;
      }
      changes.push(['remove', el]);
      ref = el.getElementsByTagName('*');
      for (j = 0, len1 = ref.length; j < len1; j++) {
        descendant = ref[j];
        changes.push(['remove', descendant]);
      }
    }
  };

  revalidateOrphanedElements = function(changes) {
    var el, i, j, len, len1, observer, ref;
    for (i = 0, len = documentObservers.length; i < len; i++) {
      observer = documentObservers[i];
      if (observer) {
        ref = observer.elements;
        for (j = 0, len1 = ref.length; j < len1; j++) {
          el = ref[j];
          if (!el.parentNode) {
            changes.push(['remove', el]);
  ...
  revalidateObservers = function(changes, node) {
    var data, i, id, ids, j, len, len1, observer, ref;
    if (node.nodeType !== Node.ELEMENT_NODE) {
      return;
    }
    ref = selectorSet.matches(node);
    for (i = 0, len = ref.length; i < len; i++) {
      data = ref[i].data;
      changes.push(['add', node, data]);
    }
    if (ids = addMap.get(node)) {
      for (j = 0, len1 = ids.length; j < len1; j++) {
        id = ids[j];
        if (observer = documentObservers[id]) {
  ...
  revalidateDescendantObservers = function(changes, node) {
    var descendant, i, len, ref;
    if (node.nodeType !== Node.ELEMENT_NODE) {
      return;
    }
    revalidateObservers(changes, node);
    ref = node.getElementsByTagName('*');
    for (i = 0, len = ref.length; i < len; i++) {
      descendant = ref[i];
  ...
  applyChanges = function(changes) {
    var el, i, len, observer, ref, type;
    for (i = 0, len = changes.length; i < len; i++) {
      ref = changes[i], type = ref[0], el = ref[1], observer = ref[2];
      if (type === 'add') {
        runInit(el, observer);
        runAdd(el, observer);
      } else if (type === 'remove') {
  ...
  $.observe = function(selector, handlers) {
    var observer;
    ...
    observer = {
      id: uid++,
      selector: selector,
      initialize: handlers.initialize || handlers.init,
      add: handlers.add,
      remove: handlers.remove,
      elements: [],
      stop: function() {
        return stopObserving(observer);
      }
    };
  ...
  scheduleAddDocumentNodes = function() {
    if (addDocumentNodesScheduled) {
      return;
    }
    setImmediate(addDocumentNodes);
    return addDocumentNodesScheduled = true;
  };

  addDocumentNodes = function() {
    var changes;
    changes = [];
    addNodes(changes, [document.documentElement]);
    applyChanges(changes);
    return addDocumentNodesScheduled = false;
  };

  $.observe.count = 0;

  $(document).on('observe:dirty', function(event) {
    var changes;
    changes = [];
    revalidateDescendantObservers(changes, event.target);
    applyChanges(changes);
  });

  changedTargets = [];

  handleAsyncChangeEvents = function(event) {
    var changes, el, els, i, j, len, len1, target, targets;
    changes = [];
  ...
  handleChangeEvent = function(event) {
    ...
    setImmediate(handleAsyncChangeEvents);
  };

  document.addEventListener('change', handleChangeEvent, false);

  $(document).on('change', handleChangeEvent);

  handleDocumentMutations = function(mutations) {
    var changes, i, len, mutation;
    changes = [];
...
(function() {
  $(document).on('ready pjax:success', function() {
    var clipper;
    clipper = new Clipboard('#copy-api-token');
    clipper.on('success', function(event) {
      var notice;
      notice = '<div class="flash notice"><div class="flash-inner">API token copied to clipboard.</div></div>';
      if ($('.flash').length > 0) {
        return $('.flash').replaceWith(notice);
      } else {
        return $('.content-push').before(notice);
      }
    });
    $('.js-address-form fieldset').addressfield({
    ...
    $('.js-edit-address').on('click', function(e) {
      e.stopPropagation();
      e.preventDefault();
      $('.js-address-form input,select').prop('disabled', false);
      $('.js-submit-address').removeClass('hidden');
      return $('.js-edit-address').addClass('hidden');
    });
    return $('.js-address-form').on('submit', function(e) {
      var $target;
      e.stopPropagation();
      e.preventDefault();
      $target = $(e.target);
      ...
      return $.ajax({
        ...
        success: function(response) {
          ...
          $('.js-address-form input,select').prop('disabled', true);
          $('.js-submit-address').addClass('hidden');
          return $('.js-edit-address').removeClass('hidden');
        },
...
(function() {
  $(document).ready(function() {
    var dragEnd;
    dragEnd = function() {
      var data, element, i, ids, len, ref;
      ids = [];
      ref = $('.js-package-sort li');
      for (i = 0, len = ref.length; i < len; i++) {
        element = ref[i];
    ...
    $('.js-admin-package-search-results').on('click', '.js-add-to-list', function(e) {
      ...
      newLi = li.clone();
      newLi.find('.js-add-to-list').removeClass('add-button js-add-to-list').addClass('delete-button js-remove-from-list').find('.octicon').removeClass('octicon-move-down').addClass('octicon-trashcan');
      $('.js-package-sort').prepend(newLi);
      li.find('.js-add-to-list').replaceWith('<span class="octicon octicon-check"></span>');
      return dragEnd();
    });
    ...
    $('.js-admin-package-search-results').on('click', '.js-admin-search-pagination a', function(e) {
      e.stopPropagation();
      e.preventDefault();
      return $('.js-admin-package-search-results').load($(e.currentTarget).attr('href'));
    });
    $('form.js-admin-package-search').submit(function(e) {
      ...
      return $.ajax({
        ...
        success: function(data) {
          return $('.js-admin-package-search-results').html(data);
        }
      });
    });
    $('.js-user-search-result .js-eligible-for-reward').on('change', function(e) {
      $('.js-user-search-result').addClass('spinnin');
      return $.ajax({
        ...
        success: function(data) {
          return $('.js-user-search-result').html(data).removeClass('spinnin');
        }
      });
    });
    ...
    $('.js-show-blacklist-modal').leanModal().on('click', function() {
      $('.js-user-blacklist-modal input').focus();
      return null;
    });
    $('.js-user-blacklist-confirm-form').on('submit', function(e) {
      var blacklist_reason;
      blacklist_reason = $('input[name=user-blacklist-reason]').val();
      if (blacklist_reason === '') {
        return false;
      }
    });
    $('.js-show-delete-modal').leanModal().on('click', function() {
      $('.js-user-delete-modal input').focus();
      return null;
    });
    return $('.js-user-delete-confirm-form').on('submit', function(e) {
      var entered;
      entered = $('input[name=user-delete-confirm]').val();
      if (!(entered === $('input[name=user-delete-confirm-username]').val() && entered !== '')) {
        return false;
      }
    });
  });
...
(function() {
  $(document).on('click', '.js-tabbed-pages .js-tabbed-tab', function(event) {
    var $container, tab;
    event.preventDefault();
    $container = $(event.currentTarget).closest('.js-tabbed-pages');
    tab = $(event.currentTarget).attr('data-page');
    $container.find('.js-tabbed-tab').filter('.selected').removeClass('selected');
    $container.find('.js-tabbed-page').filter('.selected').removeClass('selected');
    return $container.find("[data-page='" + tab + "']").addClass('selected');
  });
...
(function() {
  ...
  $(document).ready(function() {
    $('.js-search-version-select').on('selectmenu:selected', function(e) {
      var pathname_parts, versionIndex;
      if (window.location.pathname.match(/^\/docs\/api\/search/i)) {
        versionIndex = 4;
      } else if (window.location.pathname.match(/^\/docs\/api/i)) {
        versionIndex = 3;
      } else {
        versionIndex = 2;
      }
      pathname_parts = window.location.pathname.split('/');
      pathname_parts[versionIndex] = $(e.target).data('version');
      return window.location.pathname = pathname_parts.join('/');
    });
    $(document).pjax('.documents a:not([data-remote]):not([data-behavior]):not([data-skip-pjax])', '#js-pjax-container');
    if (window.location.hash) {
      return toggleApiEntry(window.location.hash);
    }
  });
  ...
  toggleApiEntry = function(apiEntryId) {
    var entry, extendedContainer;
    entry = $(apiEntryId);
    extendedContainer = entry.parents('.extended-methods-container');
    if (extendedContainer.length) {
      extendedContainer.addClass('show');
    }
    if (entry.hasClass('expanded')) {
      return entry.removeClass('expanded');
    } else {
      entry.addClass('expanded');
      return window.history.replaceState('', '', apiEntryId);
    }
  };
...
(function() {
  $(document).ready(function() {
    $('.js-show-flag').leanModal().on('click', function() {
      $('#flag-modal textarea').focus();
      return null;
    });
    $('#flag-modal').on('keydown', function(arg) {
      var keyCode, metaKey;
      keyCode = arg.keyCode, metaKey = arg.metaKey;
      switch (keyCode) {
        case 13:
          if (metaKey) {
    ...
    $('.star-box').on('click', 'a.js-star-button', function(e) {
      var target;
      if (e.target.className.match(/octicon-star/)) {
      ...
      e.stopPropagation();
      e.preventDefault();
      if (!target.className.match(/\bdisabled\b/)) {
        return $.ajax({
          ...
          success: (function(_this) {
            return function(data, textStatus, jqXHR) {
              return $(target).parent().html(data);
            };
          })(this)
        });
      }
    ...
    $('.trending-select').on('selectmenu:selected', function(e) {
      var $target, isTheme, sort, url;
      $target = $(e.target);
      sort = $target.data('trending-sort');
      isTheme = !!$target.data('is-theme');
      $('.package-list.trending .loading-overlay').show();
      if (isTheme) {
        url = "/themes/trending/" + sort;
      } else {
        url = "/packages/trending/" + sort;
      }
      return $.ajax({
        url: url,
        headers: {
    ...
    return $('.card-install-button-close').on('click', function() {
...
;
(function() {
  var isTransitioningHeight, supportsTransitions, withoutTransition;

  supportsTransitions = 'ontransitionend' in window;

  $.fn.performTransition = function(cb) {
    var $el, el, els, i, j, len, len1, transitionHeight;
    ...
    for (i = 0, len = els.length; i < len; i++) {
      ...
      $el.one('transitionend', function() {
        ...
        if (transitionHeight) {
          return withoutTransition(el, function() {
      ...
      if (transitionHeight) {
        withoutTransition(el, function() {
          return el.style.height = ($el.height()) + "px";
        });
      }
      el.offsetHeight;
    }
  ...
  isTransitioningHeight = function(el) {
    return $(el).css('transitionProperty') === 'height';
  };

  withoutTransition = function(el, cb) {
...
(function() {
  $.fn.fire = function(type) {
    var arg, defaultAction, element, props, trigger;
    if (arg = arguments[1]) {
    ...
    if (props.cancelable == null) {
      props.cancelable = !!defaultAction;
    }
    if (props.bubbles == null) {
      props.bubbles = true;
    }
    trigger = function() {
      var event;
      event = $.Event(type, props);
      $.event.trigger(event, [], element, !event.bubbles);
      if (defaultAction && !event.isDefaultPrevented()) {
...
(function() {
  var eventKeyChar, handle, keyCodeToChar, shiftKeyCodeToChar;
  ...
  eventKeyChar = function(event) {
    var char, modifiers, shiftChar;
    ...
    if (event.shiftKey) {
      if (shiftChar = shiftKeyCodeToChar[event.which]) {
  ...
  handle = function(event) {
    if (event.hotkey == null) {
      event.hotkey = eventKeyChar(event);
    }
...
(function() {
  var activate, activeContainer, deactivate, onDocumentClick, onKeyDown,
  ...
  activate = function(container) {
    ...
    $(container).fire('menu:activate', function() {
      $(document).on('keydown.menu', onKeyDown);
      $(document).on('click.menu', onDocumentClick);
      activeContainer = container;
      $(container).performTransition(function() {
        document.body.classList.add('menu-active');
        container.classList.add('active');
        return $(container).find('.js-menu-content[aria-hidden]').attr('aria-hidden', 'false');
      });
      return $(container).fire('menu:activated', {
        async: true
      });
    });
  };
  ...
  deactivate = function(container) {
    $(container).fire('menu:deactivate', function() {
      $(document).off('.menu');
      activeContainer = null;
      $(container).performTransition(function() {
        document.body.classList.remove('menu-active');
        container.classList.remove('active');
        return $(container).find('.js-menu-content[aria-hidden]').attr('aria-hidden', 'true');
      });
      return $(container).fire('menu:deactivated', {
        async: true
      });
    });
  };
  ...
  onKeyDown = function(event) {
    ...
    if (event.hotkey === 'esc') {
      if (indexOf.call($(document.activeElement).parents(), activeContainer) >= 0) {
        document.activeElement.blur();
      }
      event.preventDefault();
      deactivate(activeContainer);
  ...
  $.fn.menu = function(method) {
    ...
    methods = {
      ...
      deactivate: (function(_this) {
        return function() {
          return deactivate(container);
        };
      })(this)
    };
    return typeof methods[method] === "function" ? methods[method]() : void 0;
  };
...
(function() {
  $(document).on('selectmenu:selected', '.js-select-menu .js-navigation-item', function() {
    var container, gravatar, text;
    container = $(this).closest('.js-select-menu');
    text = $(this).find('.js-select-button-text');
    if (text[0]) {
      container.find('.js-select-button').html(text.html());
    }
    gravatar = $(this).find('.js-select-menu-item-gravatar');
    if (text[0]) {
      return container.find('.js-select-button-gravatar').html(gravatar.html());
    }
  });
...
(function() {
  $(document).on('selectmenu:change', '.js-select-menu .select-menu-list', function(event) {
    var empty, items;
    items = $(this).find('.js-navigation-item');
    items.removeClass('last-visible');
    items.visible().last().addClass('last-visible');
    if (!$(this).is('[data-filterable-for]')) {
      empty = $(event.target).hasClass('filterable-empty');
      $(this).toggleClass('filterable-empty', empty);
    }
  });

}).call(this);
(function() {
  var sortByScoreAndAlpha, stringScore;

  $.fuzzyScore = function(string, query) {
    var score;
  ...
  $.fuzzySort = function(list, query) {
    var item, j, len, results, score, str;
    list = (function() {
      var j, len, results;
      results = [];
      for (j = 0, len = list.length; j < len; j++) {
        str = list[j];
        if (score = $.fuzzyScore(str, query)) {
          results.push([str, score]);
        }
      }
      return results;
    })();
    list.sort(sortByScoreAndAlpha);
    results = [];
    for (j = 0, len = list.length; j < len; j++) {
      item = list[j];
      results.push(item[0]);
  ...
  sortByScoreAndAlpha = function(a, b) {
    var as, at, bs, bt;
    at = a[0];
    bt = b[0];
    as = a[1];
  ...
  $.fuzzyRegexp = function(query) {
    var special, specialRegexp, string;
    string = query.toLowerCase();
    special = "+.*?[]{}()^$|\\".replace(/(.)/g, '\\$1');
    specialRegexp = new RegExp("\\(([" + special + "])\\)", 'g');
    query = string.replace(/(.)/g, "($1)(.*?)").replace(specialRegexp, '(\\$1)');
    return new RegExp("(.*)" + query + "$", 'i');
  };

  $.fuzzyHighlight = function(content, text, textRe) {
    var clean, html, i, j, m, matches, open, ref;
    if (textRe == null) {
      textRe = null;
    }
    html = $.trim(content.innerHTML);
    if (text) {
      if (textRe == null) {
        textRe = $.fuzzyRegexp(text);
      }
      if (!(matches = html.match(textRe))) {
        return;
      }
      open = false;
      html = [];
      for (i = j = 1, ref = matches.length; 1 <= ref ? j < ref : j > ref; i = 1 <= ref ? ++j : --j) {
        m = matches[i];
        if (!m) {
          continue;
        }
  ...
  stringScore = function(string, abbreviation) {
    var abbreviation_length, abbreviation_score, c, character_score, final_score, i, index_c_lowercase, index_c_uppercase, index_in_string, j, len, min_index, should_award_common_prefix_bonus, string_length, total_character_score;
    ...
    for (i = j = 0, len = abbreviation.length; j < len; i = ++j) {
      c = abbreviation[i];
...
(function() {
  var defaultContent, defaultMarker, defaultText, originalOrder, sortByScore;

  originalOrder = new WeakMap;

  $.fn.fuzzyFilterSortList = function(query, options) {
    var allItems, c, children, content, i, item, items, j, k, l, len, len1, len2, len3, limit, list, mark, queryRe, ref, ref1, ref2, ref3, results, score, text, total, visible;
    if (options == null) {
      options = {};
    }
    if (!(list = this[0])) {
    ...
    content = (ref = options.content) != null ? ref : defaultContent;
    text = (ref1 = options.text) != null ? ref1 : defaultText;
    score = (ref2 = options.score) != null ? ref2 : $.fuzzyScore;
    limit = options.limit;
    if (options.mark === true) {
      mark = defaultMarker;
    } else if (((ref3 = options.mark) != null ? ref3.call : void 0) != null) {
      mark = options.mark;
    }
    if (allItems = originalOrder.get(list)) {
      children = $(list).children();
    } else {
      children = allItems = $(list).children();
      originalOrder.set(list, allItems.slice(0));
    }
    ...
    visible = 0;
    if (!query) {
      for (j = 0, len1 = allItems.length; j < len1; j++) {
        item = allItems[j];
      ...
      for (k = 0, len2 = items.length; k < len2; k++) {
        item = items[k];
        if (item.fuzzyFilterTextCache == null) {
          item.fuzzyFilterTextCache = text(content(item));
        }
        item.fuzzyFilterScoreCache = score(item.fuzzyFilterTextCache, query);
      }
      items.sort(sortByScore);
      queryRe = $.fuzzyRegexp(query);
      for (l = 0, len3 = items.length; l < len3; l++) {
        item = items[l];
        if ((!limit || total < limit) && item.fuzzyFilterScoreCache > 0) {
          visible++;
          if (mark) {
            c = content(item);
            mark(c);
  ...
  defaultContent = function(item) {
    return item;
  };

  defaultText = function(content) {
    return $.trim(content.textContent.toLowerCase());
  };
...
(function() {
  var defaultMarker, defaultText;

  $.fn.prefixFilterList = function(query, options) {
    var i, item, items, len, limit, list, mark, ref, ref1, text, visible;
    if (options == null) {
      options = {};
    }
    if (!(list = this[0])) {
    ...
    query = query.toLowerCase();
    text = (ref = options.text) != null ? ref : defaultText;
    items = $(list).children();
    limit = options.limit;
    if (options.mark === true) {
      mark = defaultMarker;
    } else if (((ref1 = options.mark) != null ? ref1.call : void 0) != null) {
      mark = options.mark;
    }
    visible = 0;
    for (i = 0, len = items.length; i < len; i++) {
      item = items[i];
      if (text(item).indexOf(query) === 0) {
  ...
  defaultText = function(item) {
    return $.trim(item.textContent.toLowerCase());
  };

  defaultMarker = function(item, text) {
    var clean, html, re;
    html = item.innerHTML;
    if (text) {
...
(function() {
  ...
  $.fn.substringFilterList = function(query, options) {
    var i, item, items, len, limit, list, mark, ref, ref1, text, visible;
    if (options == null) {
      options = {};
    }
...
(function() {
  $.fn.focused = function(selector) {
    var context, onEnter, onExit;
    onEnter = [];
    onExit = [];
    if (selector) {
      context = this.find(selector).filter(document.activeElement)[0];
    } else {
      context = this.filter(document.activeElement)[0];
    }
    ...
    this.on('focusout', selector, function() {
      var fn, i, len;
      if (context) {
        context = null;
        for (i = 0, len = onExit.length; i < len; i++) {
          fn = onExit[i];
          fn.call(this);
        }
      }
...
(function() {
  ...
  setup = function() {
    var delay, inputed, keypressed, schedule, timer;
    keypressed = false;
    inputed = false;
    timer = null;
    delay = 100;
    schedule = (function(_this) {
      return function(target) {
        ...
        timer = setTimeout(function() {
          var event;
          timer = null;
          event = new $.Event('throttled:input', {
            target: target
          });
          $.event.trigger(event, null, _this, true);
        }, delay);
      };
    })(this);
    ...
    $(this).on('keyup.throttledInput', function(event) {
      keypressed = false;
      if (inputed) {
    ...
    return $(this).on('input.throttledInput', function(event) {
      inputed = true;
      if (!keypressed) {
        schedule(event.target);
      }
  ...
  teardown = function() {
    ...
    return $(this).off('input.throttledInput');
  };
...
(function() {
  ...
  $(document).focused('.js-filterable-field')["in"](function() {
    ...
    $(this).on('throttled:input.filterable', function() {
      ...
      value = $(this).val();
      return $(this).fire('filterable:change', {
        async: true
      });
    });
    return $(this).fire('filterable:change', {
  ...
  $(document).on('filterable:change', '.js-filterable-field', function() {
    var $list, i, len, list, query, ref;
    query = $.trim($(this).val().toLowerCase());
    ref = $("[data-filterable-for=" + this.id + "]");
    for (i = 0, len = ref.length; i < len; i++) {
      list = ref[i];
      $list = $(list);
      filter($list, query);
      $list.fire('filterable:change', {
        relatedTarget: this
      });
    }
  });
  ...
  filter = function(list, query) {
    var content, limit, mark, visible;
    if (list.find('.js-filterable-text')[0]) {
      content = function(item) {
        return $(item).find('.js-filterable-text')[0];
      };
    }
    mark = list.attr('data-filterable-highlight') !== void 0;
    limit = list.attr('data-filterable-limit');
    visible = (function() {
      switch (list.attr('data-filterable-type')) {
        case 'fuzzy':
          return list.fuzzyFilterSortList(query, {
            content: content,
            mark: mark,
            limit: limit
          });
        case 'substring':
          return list.substringFilterList(query, {
            content: content,
            mark: mark,
            limit: limit
          });
        default:
          return list.prefixFilterList(query, {
            content: content,
            mark: mark,
            limit: limit
          });
    ...
    list.toggleClass('filterable-empty', visible === 0);
  };
...
(function() {
  $(document).on('navigation:open', '.js-select-menu:not([data-multiple]) .js-navigation-item', function() {
    var $container, $el;
    $el = $(this);
    $container = $el.closest('.js-select-menu');
    $container.find('.js-navigation-item.selected').removeClass('selected');
    $el.addClass('selected');
    $el.removeClass('indeterminate');
    $el.find('input[type=radio], input[type=checkbox]').prop('checked', true).change();
    $el.fire('selectmenu:selected');
    if (!$container.hasClass('is-loading')) {
      return $container.menu('deactivate');
    }
  });
  ...
  $(document).on('navigation:open', '.js-select-menu[data-multiple] .js-navigation-item', function() {
    ...
    $el.removeClass('indeterminate');
    $el.find('input[type=radio], input[type=checkbox]').prop('checked', !selected).change();
    return $el.fire('selectmenu:selected');
  });
...
(function() {
  ...
  loadContents = function(event) {
    var $el, data, el, url, xhr;
    el = event.currentTarget;
    $el = $(el);
    $el.removeClass('js-load-contents');
    $el.addClass('is-loading');
    $el.removeClass('has-error');
    url = $el.attr('data-contents-url');
    data = $el.data('contents-data');
    xhr = $.ajax({
      url: url,
      data: data
    });
    return xhr.then(function(data) {
      ...
      $el.find('.js-select-menu-deferred-content').html(data).pageUpdate();
      if ($el.hasClass('active')) {
        $el.fire('selectmenu:load');
      }
    }, function() {
      $el.removeClass('is-loading');
  ...
  $.observe('.js-select-menu.js-load-contents', {
    add: function() {
      $(this).on('mouseenter', loadContents);
      $(this).on('menu:activate', loadContents);
    },
    remove: function() {
      $(this).off('mouseenter', loadContents);
      $(this).off('menu:activate', loadContents);
    }
  });

}).call(this);
(function() {
  $.fn.positionedOffset = function(container) {
    var bottom, element, height, left, right, scrollHeight, scrollWidth, top, width;
    if (!(element = this[0])) {
      return;
    }
    if (container != null ? container.jquery : void 0) {
...
(function() {
  ...
  $.fn.scrollTo = function() {
    var args, container, left, options, ref, ref1, top;
    ...
    if ((options.top == null) && (options.left == null)) {
      if (options.target) {
        ref = $(options.target).positionedOffset(container), top = ref.top, left = ref.left;
        options.top = top;
        options.left = left;
      } else {
        ref1 = $(container).positionedOffset(), top = ref1.top, left = ref1.left;
        options.top = top;
        options.left = left;
        container = document;
      }
    ...
    if (!container.offsetParent) {
      if (options.duration) {
        ...
        if (options.top != null) {
          $(document).scrollTop(options.top);
        }
        if (options.left != null) {
          $(document).scrollLeft(options.left);
        }
  ...
  animateScroll = function(container, options) {
    var count, opts, props;
    props = {};
    if (options.top != null) {
      props.scrollTop = options.top;
    }
    if (options.left != null) {
      props.scrollLeft = options.left;
    }
    opts = {
      duration: options.duration,
      queue: false
    ...
    if (options.complete) {
      ...
      opts.complete = function() {
        if (--count === 0) {
          return setImmediate(options.complete);
        }
      };
    }
    return $(container).animate(props, opts);
  };

}).call(this);
(function() {
  $.fn.overflowOffset = function(container) {
    var bottom, element, height, left, offset, right, scroll, top, width;
    if (container == null) {
    ...
    if (!(offset = $(element).positionedOffset(container))) {
      return;
    }
    if (container.offsetParent) {
      scroll = {
...
(function() {
  $.fn.overflowParent = function() {
    var element, overflowX, overflowY;
    if (!(element = this[0])) {
      return $();
    }
    ...
    while (element !== document.body) {
      ...
      if (overflowY === 'auto' || overflowX === 'auto' || overflowY === 'scroll' || overflowX === 'scroll') {
        break;
      }
    }
    return $(element);
...
(function() {
  var activate, bindNavigationMouseListeners, clear, containerStack, ctrlBindings, cursorDown, cursorUp, deactivate, disableMouseEvents, fireOpen, focus, focusItem, getActiveContainer, getItems, getScrollStyle, keyOpen, modifierKey, mousePosition, onContainerMouseMove, onContainerMouseOver, pageDown, pageUp, pop, push, refocus, scrollItemTo, scrollPageTo, unbindNavigationMouseListeners;

  ctrlBindings = navigator.userAgent.match(/Macintosh/);

  modifierKey = navigator.userAgent.match(/Macintosh/) ? 'meta' : 'ctrl';
  ...
  bindNavigationMouseListeners = function(el) {
    el.addEventListener('mousemove', onContainerMouseMove, false);
    el.addEventListener('mouseover', onContainerMouseOver, false);
  };

  unbindNavigationMouseListeners = function(el) {
    el.removeEventListener('mousemove', onContainerMouseMove, false);
    el.removeEventListener('mouseover', onContainerMouseOver, false);
  };

  $.observe('.js-navigation-container', {
    add: bindNavigationMouseListeners,
    remove: unbindNavigationMouseListeners
  });
  ...
  onContainerMouseOver = function(event) {
  ...
  $(document).on('keydown', function(event) {
    var container, customEvent, target;
    if (event.target !== document.body && !event.target.classList.contains('js-navigation-enable')) {
      return;
    }
    if (!(container = getActiveContainer())) {
      return;
    }
    disableMouseEvents = true;
    target = $(container).find('.js-navigation-item.navigation-focus')[0] || container;
  ...
  $(document).on('navigation:keydown', '.js-active-navigation-container', function(event) {
    var container, isInput, item;
    container = this;
    isInput = $(event.originalEvent.target).is('input, textarea');
  ...
  $(document).on('navigation:mouseover', '.js-active-navigation-container .js-navigation-item', function(event) {
    var container;
    container = $(event.currentTarget).closest('.js-navigation-container')[0];
    focusItem(event.currentTarget, container);
  });

  fireOpen = function(originalEvent) {
    var event, modifier, target;
    target = originalEvent.currentTarget;
    modifier = originalEvent.modifierKey || originalEvent.altKey || originalEvent.ctrlKey || originalEvent.metaKey;
    event = $(target).fire('navigation:open', {
  ...
  $(document).on('click', '.js-active-navigation-container .js-navigation-item', function(event) {
    fireOpen(event);
  });

  $(document).on('navigation:keyopen', '.js-active-navigation-container .js-navigation-item', function(event) {
    var a;
    if (a = $(this).filter('.js-navigation-open')[0] || $(this).find('.js-navigation-open')[0]) {
  ...
  activate = function(container) {
    ...
    if (container !== activeContainer) {
      return $(container).fire('navigation:activate', (function(_this) {
        return function() {
          if (activeContainer) {
            activeContainer.classList.remove('js-active-navigation-container');
          }
          container.classList.add('js-active-navigation-container');
          return $(container).fire('navigation:activated', {
            async: true
          });
        };
      })(this));
  ...
  deactivate = function(container) {
    return $(container).fire('navigation:deactivate', (function(_this) {
      return function() {
        container.classList.remove('js-active-navigation-container');
        return $(container).fire('navigation:deactivated', {
          async: true
        });
      };
    })(this));
  ...
  focus = function(target, container) {
    var firstItem, focusPrevented, item;
    firstItem = getItems(container)[0];
    item = $(target).closest('.js-navigation-item')[0] || firstItem;
    activate(container);
    if (item) {
  ...
  refocus = function(target, container) {
    clear(container);
    focus(target, container);
  };

  cursorUp = function(item, container) {
    var focusPrevented, index, items, previous;
  ...
  cursorDown = function(item, container) {
    var focusPrevented, index, items, next;
    items = getItems(container);
    index = $.inArray(item, items);
  ...
  pageUp = function(item, container) {
    var focusPrevented, index, items, previous;
    items = getItems(container);
    index = $.inArray(item, items);
    container = $(item).overflowParent()[0];
    while ((previous = items[index - 1]) && $(previous).overflowOffset(container).top >= 0) {
      index--;
    }
    if (previous) {
      focusPrevented = focusItem(previous, container);
  ...
  pageDown = function(item, container) {
    var focusPrevented, index, items, next;
    items = getItems(container);
    index = $.inArray(item, items);
    container = $(item).overflowParent()[0];
    while ((next = items[index + 1]) && $(next).overflowOffset(container).bottom >= 0) {
      index++;
    }
    if (next) {
      focusPrevented = focusItem(next, container);
  ...
  keyOpen = function(item, modifier) {
    if (modifier == null) {
      modifier = false;
    }
    $(item).fire('navigation:keyopen', {
      modifierKey: modifier
    });
  };

  focusItem = function(target, container) {
    var event;
    event = $(target).fire('navigation:focus', function() {
      clear(container);
      target.classList.add('navigation-focus');
      return $(target).fire('navigation:focused', {
        async: true
      });
    });
    return event.isDefaultPrevented();
  ...
  getItems = function(container) {
    return $(container).find('.js-navigation-item').visible();
  };

  getScrollStyle = function(container) {
    var ref;
    return (ref = $(container).attr('data-navigation-scroll')) != null ? ref : 'item';
  };

  scrollPageTo = function(container, element) {
    var overflow, position, scrollHeight, scrollTop;
    ...
    if (overflow.bottom <= 0) {
      ...
      scrollHeight = container.offsetParent != null ? container.scrollHeight : $(document).height();
  ...
  scrollItemTo = function(container, element) {
    var overflow, position, scrollHeight, scrollTop;
    position = $(element).positionedOffset(container);
    overflow = $(element).overflowOffset(container);
    if (overflow.bottom <= 0) {
  ...
  $.fn.navigation = function(method) {
    ...
    if (!(container = $(this).closest('.js-navigation-container')[0])) {
      return;
    }
    methods = {
      activate: (function(_this) {
      ...
      refocus: (function(_this) {
        return function() {
          return refocus(_this, container);
        };
      })(this)
    };
    return typeof methods[method] === "function" ? methods[method]() : void 0;
...
(function() {
  $(document).on('menu:activate', '.js-select-menu', function() {
    ...
    $(this).find('.js-menu-target').attr('aria-expanded', true).addClass('selected');
    return $(this).find('.js-navigation-container').navigation('push');
  });

  $(document).on('menu:deactivate', '.js-select-menu', function() {
    $(this).find('.js-menu-target').attr('aria-expanded', false).removeClass('selected');
    return $(this).find('.js-navigation-container').navigation('pop');
  });
...
(function() {
  ...
  $(document).on('filterable:change', '.js-select-menu .select-menu-list', function(event) {
    var filterText, form;
    if (form = $(this).find('.js-new-item-form')[0]) {
      filterText = event.relatedTarget.value;
      if (filterText === "" || itemExists(this, filterText)) {
        $(this).removeClass('is-showing-new-item-form');
      } else {
        $(this).addClass('is-showing-new-item-form');
        $(form).find('.js-new-item-name').text(filterText);
        $(form).find('.js-new-item-value').val(filterText);
      }
    }
    $(event.target).trigger('selectmenu:change');
  });

  itemExists = function(container, filterText) {
    var i, item, len, ref, text;
    ref = $(container).find('.js-select-button-text');
    for (i = 0, len = ref.length; i < len; i++) {
      item = ref[i];
...
(function() {
  ...
  $(document).on('menu:activate selectmenu:load', '.js-select-menu', function() {
    var $tab;
    $tab = $(this).find('.js-select-menu-tab').first();
    return $tab.addClass('selected');
  });

  $(document).on('click', '.js-select-menu .js-select-menu-tab', function(event) {
    var $menu;
    $menu = $(this).closest('.js-select-menu');
    $menu.find('.js-select-menu-tab').removeClass('selected');
    $(this).addClass('selected');
    return false;
  });

  toggleTabBucket = function(tab, state) {
    var $bucket, $buckets, name;
    name = tab.getAttribute('data-tab-filter');
    $buckets = $(tab).closest('.js-select-menu').find('.js-select-menu-tab-bucket');
    $bucket = $buckets.filter(function() {
      return this.getAttribute('data-tab-filter') === name;
    });
    $bucket.toggleClass('selected', state);
    if (state) {
      $bucket.fire('selectmenu:tabchange');
    }
  };

  $.observe('.js-select-menu .js-select-menu-tab.selected', {
    add: function() {
      return toggleTabBucket(this, true);
    },
    remove: function() {
      return toggleTabBucket(this, false);
    }
  });
...
(function() {
  $(document).ready(function() {
    return $('.welcome').on('click', '.js-download-button', function(e) {
      var error, href, match;
      href = $(e.currentTarget).closest('a').attr('href');
      match = /\/download\/(\w+)/.exec(href);
      if ((match != null) && typeof ga !== 'undefined') {
        ga('send', 'event', 'button', 'download', match[1]);
      }
...
(function ($) {
  ...
  $.fn.addressfield.apply = function (config, fieldMap) {
    ...
    for (fieldPos in config.fields) {
      ...
      //console.log(config.fields, field, selector, $element);
      //console.log(field, fieldMap, selector);
    ...
    $.each(fieldMap, function (field_name, field_selector) {
      ...
      if ($element.length && !$.fn.addressfield.hasField(config, field_name)) {
        console.log("hiding?", $element, field_name);
		...
		this.each(function(i, cont) {
			...
			var newList = {
				...
				styleDragHandlers: function(cursor) {
					this.getItems().map(function() { return $(this).is(opts.dragSelector) ? this : $(this).find(opts.dragSelector).get(); }).css("cursor", cursor ? "grab" : "");
				},
